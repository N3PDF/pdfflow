...
class mkPDF:
    ...
    def _xfxQ2(self, u, aa_x, aa_q2):
        ...
        for subgrid in self.subgrids:
            ...
            valid = tf.math.logical_and(a_x >= subgrid.log_xmin,\
                                       a_x <= subgrid.log_xmax\
                                       )
            stripe = tf.math.logical_and(a_q2 >= subgrid.log_q2min,\
                                        a_q2 <= subgrid.log_q2max\
                                        )
        ...
        #--------------------------------------------------------------------
        #low q2
        x_stripe = tf.math.logical_and(a_x >= self.subgrids[0].log_xmin,\
                                      a_x <= self.subgrids[0].log_xmax\
                                      )
        ...
        #high q2
        x_stripe = tf.math.logical_and(a_x >= self.subgrids[-1].log_xmin,\
                                      a_x <= self.subgrids[-1].log_xmax\
                                      )
        ...
        #--------------------------------------------------------------------
        #low x high q2
        stripe = tf.math.logical_and(a_x < self.subgrids[-1].log_xmin,\
                                     a_q2 > self.subgrids[-1].log_q2max\
                                     )
        ...
        #--------------------------------------------------------------------
        #low x low q2
        stripe = tf.math.logical_and(a_x < self.subgrids[0].log_xmin,
                                     a_q2 < self.subgrids[0].log_q2min\
                                     )
        f_idx = tf.where(stripe)
